...
- [ ] **A07** — 不通过复制已有服务代码创建新服务；如确需复制，必须在 ADR 或设计文档中说明复用边界与差异
...
- [ ] **FE06** — 不在页面组件中重复实现权限逻辑；权限判断统一走路由 meta、access 工具与后端授权
- [ ] **FE07** — 不在页面组件中直接拼接业务 API URL；统一走 `src/api/` 模块的封装函数
- [ ] **FE08** — 页面必须显式覆盖权限不足（Forbidden / 403）状态；不能仅靠路由隐藏
- [ ] **FE09** — 禁止引入第二套 UI 体系或第二套状态管理；统一使用 Vben 组件与 Pinia

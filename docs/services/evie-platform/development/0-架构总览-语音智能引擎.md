...
├── pkg/
│   ├── asr/                                ← 🔷 ASR 供应商客户端（公共）
│   │   ├── provider.go     ASRProvider 接口 + RecognizeOptions
│   │   ├── funasr/         FunASR gRPC 客户端
│   │   ├── whisper/        Whisper 客户端
│   │   ├── xunfei/         讯飞实时语音转写客户端
│   │   └── aliyun/         阿里云语音识别客户端
│   ├── llm/                                ← 🔷 LLM 供应商客户端（公共）
...
│   │   └── openai/         OpenAI 兼容客户端
│   ├── entgo/mixin/                        ← 🔷 Ent Mixin 公共层（已有）
│   └── datasource/                         ← ⚠️ 数据源拉取器（可延后）
│       ├── fetcher.go      DataFetcher 接口
│       ├── httpapi/        HTTP API 拉取
│       ├── csv/            CSV 上传解析
│       └── db/             数据库直连
│
...
│   │   ├── provider.go   # ASR 供应商管理
│   │   ├── data_source.go# 字典数据源集成
│   │   └── entity.go     # 实体识别接口
│   ├── biz/              # 业务逻辑层
│   │   ├── asr.go        # ASR 编排（采集→预处理→路由Provider→识别）
│   │   ├── provider.go   # Provider 注册/路由/选择
│   │   ├── correction.go # 纠错引擎（规则→拼音→实体→LLM）
│   │   ├── dictionary.go # 字典匹配、别名生成
│   │   ├── data_source.go# 数据源拉取/转换/同步
│   │   ├── hotword.go    # 热词管理
│   │   └── entity.go     # 实体识别编排
│   ├── data/             # 数据访问层
...
│   │   ├── hotword.go    # 热词 repo
│   │   ├── provider_config.go # Provider 配置 repo
│   │   ├── data_source_repo.go # 数据源配置 repo
│   │   └── ent/          # Ent 数据层
...
| ASR 引擎 | `pkg/asr/` 公共客户端（FunASR / Whisper / 讯飞 / 阿里云） |
| LLM 调用 | `pkg/llm/` 公共客户端（Qwen / DeepSeek / OpenAI 兼容） |
| 数据源拉取 | `pkg/datasource/` 公共拉取器（HTTP / CSV / DB / gRPC） |
...
├── provider.proto         # ASR 供应商管理 RPC
├── data_source.proto      # 字典数据源集成 RPC
├── common.proto           # 公共消息定义

...
| 1.7 | Biz 层：ASR 编排（采集→预处理→识别→热词增强） | 3d | 1.6 |
| 1.8 | Biz 层：字典基本 CRUD | 1d | 1.6 |
| 1.9 | Service 层：ASR Recognize、StreamRecognize | 1d | 1.7 |
| 1.10 | Service 层：热词 CRUD | 0.5d | 1.8 |
| 1.11 | Wire 依赖注入组装 | 0.5d | 1.9 |
| 1.12 | FunASR Server 部署 + Go Client 对接 | 2d | 1.7 |
| 1.13 | 音频预处理（降噪/VAD）基本实现 | 1.5d | 1.7 |
| 1.14 | 接入平台认证（JWT 提取 tenant_id） | 1d | 1.9 |
| 1.15 | Mock 数据（2 个租户，各 50+ 字典词） | 1d | 1.6 |
| 1.16 | `make api` / `make ent` / `make wire` 集成到 CI | 0.5d | 1.11 |

### 2.2 前端

| # | 任务 | 工作量 |
|---|------|:---:|
| 1.17 | API 文件：`src/api/evie/asr.ts`、`src/api/evie/dictionary.ts` | 1d |
| 1.18 | 字典管理页：list + 新增/编辑 drawer | 2d |
| 1.19 | 热词管理页：list + 新增/编辑 drawer | 1d |
| 1.20 | 语音输入组件（录制/停止/波形） | 2d |
| 1.21 | ASR 结果展示页（原始文本 + 置信度） | 1d |
| 1.22 | Locale 中英文 | 0.5d |
| 1.23 | 路由 + 菜单注册 | 0.5d |
...
| V2 | 租户隔离 | 租户 A 的热词不影响租户 B |
| V3 | 基础纠错 | 字典中的固定词能准确替换 |

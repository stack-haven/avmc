...
日期：2026-06-16  
状态：生效  
范围：`backend-service`、`docs/services`、`docs/product`、后续项目服务后端模块开发

## 1. 决策摘要

项目开发底座后端当前采用“go-kratos 大仓 + 模块化服务边界优先”策略。应用版本管理中心（AVMC）是当前底座上的第一个业务项目服务，但不能再和底座管理后台基础服务混在一起。
...
- `backend-service/app/version/service` 当前冻结，只保留已存在雏形，不继续承接迭代 1/2 新业务。
- AVMC 版本管理、Release、灰度、下载页、反馈、协议、推送等业务能力不再继续进入 `backend-service/app/platform/admin`。
- AVMC 业务服务需要重新定义后端服务边界；迭代 3 前复审 `version/service` 是删除、迁移、保留，还是升级为正式业务服务。

## 2. 决策背景

当前仓库的目标是开放式项目开发底座：在基础 SaaS 架构之上扩展多业务场景服务模块。AVMC 是当前第一个项目服务，主链路仍处于项目、权限、版本、Release 的早期阶段。

上一阶段曾将 `app/avmc/admin` 作为 AVMC 管理后台主服务，这会让底座被具体业务污染。当前已经将该服务迁移到 `app/platform/admin`，并升级为底座管理后台基础服务，业务项目服务另行定义。

继续把 AVMC 业务写入 `app/platform/admin` 会带来以下问题：
...
因此当前优先保证底座基础服务纯粹，再为 AVMC 定义独立业务服务边界。
...
| AVMC 灰度、下载页、反馈、协议、推送 | 待定义 AVMC 业务服务边界 | 待定义 | 不再进入 `app/platform/admin` |
...
- 不继续把 AVMC 业务模块写入 `backend-service/app/platform/admin`。
- 不继续扩展 `backend-service/app/version/service` 的 Release 雏形。
- 不把 AVMC 版本管理、Release、灰度、下载页、反馈、协议、推送落到 `app/platform/admin`。
...
- AVMC 业务服务后端落点是新服务、`version/service` 升级，还是其他路径。

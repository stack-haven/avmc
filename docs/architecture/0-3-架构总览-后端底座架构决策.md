...
- `backend-service/app/platform/service` 升级为 Ark Tech Platform 管理后台基础服务，负责认证、用户、角色、菜单、权限、中台基础配置和产品服务配置入口。
- `platform/service` 可以管理产品服务的注册、配置、菜单和权限入口，但不得承接产品服务自身的业务生命周期。
...
- 业务服务（如版本管理、Release、灰度、下载页、反馈、协议、推送等）不再进入 `backend-service/app/platform/service`。
...
上一阶段曾将业务管理服务和底座管理后台混在一起，这让底座被具体业务污染。当前已经将该服务迁移到 `app/platform/service`，并升级为底座管理后台基础服务，业务产品服务另行定义。

继续把业务模块写入 `app/platform/service` 会带来以下问题：
...
| 服务 | 当前角色 | 当前状态 | 默认用途 |
| --- | --- | --- | --- |
| `backend-service/app/platform/service` | 底座管理后台基础服务 | active | 认证、用户、角色、菜单、权限、中台基础配置、产品服务配置入口 |
| `backend-service/app/ai/service` | 底座 AI/chat 能力服务 | active | AI/chat 通用能力，具体业务接入需另行定义 |
| `backend-service/app/evie/service` | Evie ASR 产品服务 | active | 语音识别、词库中心、文本增强引擎；服务前缀 `/evie/v1` |
...
| 模块 | 默认后端落点 | 默认前端落点 | 备注 |
| --- | --- | --- | --- |
| 认证、用户、角色、菜单、部门、岗位 | `app/platform/service` | `apps/web-antd-admin` | 底座管理后台基础能力 |
| 中台权限菜单分配 | `app/platform/service` | `apps/web-antd-admin` | 底座管理后台基础能力 |
| 业务中台基础配置 | `app/platform/service` | `apps/web-antd-admin` | 底座管理后台基础能力 |
| 产品服务注册/配置入口 | `app/platform/service` | `apps/web-antd-admin` | 底座管理后台基础能力 |
| 版本管理 MVP | 待定义业务服务边界 | 待定义 | 不再进入 `app/platform/service` |
| Release 管理后台 | 待定义业务服务边界 | 待定义 | 不再进入 `app/platform/service` |
| 客户端版本检查 API | 迭代 3 复审 | 按需 | 可能成为 `version/service` 或新业务服务 |
| 灰度、下载页、反馈、协议、推送 | 待定义业务服务边界 | 待定义 | 不再进入 `app/platform/service` |
...
- 不继续把业务模块写入 `backend-service/app/platform/service`。
- 不继续扩展 `backend-service/app/version/service` 的 Release 雏形。
- 不把业务服务（版本管理、Release、灰度、下载页、反馈、协议、推送等）落到 `app/platform/service`。
- 不把产品服务业务页面放入 Vben 示例应用。
- 不以 `backend-service-pkg-bakup` 作为活跃实现依据。

### 4.2 允许项

- 可以在 `app/platform/service` 内补充底座管理后台基础能力：认证、用户、角色、菜单、权限、部门、岗位、中台基础配置、产品服务配置入口、操作日志。
...
新增模块必须同时满足 `24-产品服务模块接入规范.md` 的接入清单。缺少租户边界、权限码、Mock 数据或测试的模块，不允许作为默认迭代进入 `platform/service`。
...
```text
backend-service/app/platform/service/internal/data/ent/
...
- `app/platform/service` 改名后的 Go import、proto 生成、Wire、配置和 CI 链路是否全部同步。

...
flowchart TD
    ...
    Q1 -->|"是"| Combined["gRPC + HTTP 合一<br/>（platform/service 模式）"]
    Q1 -->|"否"| Q2
    Q2 -->|"是"| Combined
    Q2 -->|"否"| Q3
...
| 代表性服务 | `platform/service`、Evie | `version/service` |
...
| 层级 | 文件 | 命名约定 |
|------|------|---------|
| Proto | `proto/platform/service/v1/xxx.proto` | message + service + HTTP 注解合一（无 i_ 前缀） |
...
采用**合一模式**：一个服务一个 proto 文件，message + service + HTTP 注解同文件定义。

```protobuf
// proto/platform/service/v1/xxx.proto — message + service + HTTP 合一
service XxxService {
  rpc CreateXxx(CreateXxxRequest) returns (CreateXxxResponse) {
    option (google.api.http) = { post: "/platform/v1/xxx" body: "*" };
  }
  rpc ListXxxs(ListXxxsRequest) returns (ListXxxsResponse) {
    option (google.api.http) = { get: "/platform/v1/xxx" };
  }
}
```

> 历史上曾将 HTTP 与 gRPC 分开设计（`core/service/v1` + `platform/admin/v1/i_xxx`），后因价值低、复杂度高已合并为 `platform/service/v1` 单目录合一模式（详见 4-6 变更记录 2026-08-25）。

...
graph LR
    subgraph Backend["backend-service (Go/Kratos 大仓)"]
        Admin["app/platform/service<br/>🟢 活跃<br/>平台基础能力"]
...
| # | 模块 | 阶段 | 当前状态 | 核心职责 |
|:---:|------|:---:|:---:|---------|
| 1 | 统一租户底座 | 🟢 P0 | 🔄 重构后复验中 | 多租户 + 多产品叠加、生命周期、数据隔离 |
| 2 | 统一认证与权限 | 🟢 P0 | 🔄 联动复验中 | JWT/会话、RBAC、Casbin、平台/租户身份分离 |
| 3 | 租户菜单权限组与配额 | 🟢 P0 | 🔄 重构后复验中 | 菜单权限组版本化、租户绑定、角色有效菜单、资源额度 |
...
| 11 | Webhook 与事件中心 | 🟠 P2 | ✅ MVP 已验证 | 事件订阅、签名投递、失败重试、失败记录重放 |
...
| ADR | 决策 | 理由 |
|:---:|------|------|
| **ADR-001** | `platform/service` 为平台基础服务（原 `platform/admin`），产品业务不入此服务 | 防止基础服务被业务逻辑污染 |
...
```
docs/architecture/
├── 00-04  总览层           ← 架构全景、分层定义、技术栈、工程治理总纲（原则/边界/Agent 决策权限/协作模式）
...
**工程治理与 Agent 协作入口**：[`0-4-架构总览-工程治理总纲.md`](./0-4-架构总览-工程治理总纲.md) + [`.agents/AGENTS.md`](../../.agents/AGENTS.md) + [`.agents/skills/agent-decision-boundary`](../../.agents/skills/agent-decision-boundary/SKILL.md)。
